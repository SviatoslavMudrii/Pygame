...
# ================== ЯБЛУКО З РІДКИМ СПАВНОМ ==================
class Apple:
    def __init__(self):
        self.reset()
        self.spawn_timer = 0
        self.spawn_delay = 300   # 300 кадрів = 5 секунд
        self.active = False
    ...
    def update(self, speed):
        # якщо яблука нема – чекаємо таймер
        if not self.active:
            self.spawn_timer += 1
...
while run:
    ...
    if state == "menu":
        ...
        btn_play = draw_button("Грати", 300, 300, 200, 60, (50, 150, 50))
        btn_skin = draw_button("Одежа", 300, 400, 200, 60, (100, 100, 100))
    ...
    # ================== GAME OVER ==================
        ...
        window.fill((50, 0, 0))
        txt_over = font_menu.render("Куда ти жинеш?, може щераз?", True, (255, 255, 255))
        window.blit(txt_over, (WIDTH // 2 - txt_over.get_width() // 2, 100))

        btn_retry = draw_button("ЗАНОВО", 300, 250, 200, 60, (50, 150, 50))
        btn_to_menu = draw_button("В МЕНЮ", 300, 350, 200, 60, (150, 50, 50))
